...
import { IErrorProps, IUIProps, UxUtils } from "@sharedUI";
import "./ChoiceContainer.scss";
import { InputBox } from "../InputBox";
import { Icon, Text, ShorthandValue, IconProps, Flex } from "@stardust-ui/react";
...
export interface IChoiceContainerProps extends IErrorProps, IUIProps {
    title?: string;
...
export class ChoiceContainer extends React.PureComponent<IChoiceContainerProps> {
    ...
    getDeleteIconProps(i: number): ShorthandValue<IconProps> {
        if (this.props.options.length > 2) {
            return {
                name: "trash-can",
                className: "choice-trash-can",
                outline: true,
                'aria-hidden': 'false',
            ...
            <div
                ...
                {this.props.options.length < maxOptions &&
                    <div ref={(e) => {
                        ...
                        <Icon className="plus-icon" name="add" outline size="medium" color="brand" />
